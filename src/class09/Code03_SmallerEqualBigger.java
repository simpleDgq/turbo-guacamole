...
public class Code03_SmallerEqualBigger {
	 ...
	 * 2. 遍历链表，如果当前元素小于划分值；1）如果小头为空，则将小头小尾都指向该节点；否则将小尾的next指向该节点，同时小尾指向该元素
	 * 同理，如果当前元素等于或大于该元素的时候也一样处理
	 * 3. 将三个区域链接起来，小尾连等头，等尾连大头。需要注意的事，这三个区域都有可能为空。
	 * 
	 * 最后连接思路：
	 * 如果有小于区，则小于区的尾指向等于区的头（如果没有等于区，则会指向null，并不影响）
	 * 然后判断等于区的尾是不是null，如果是null，说明没有等于区，将ET变成小于区的尾LT，
	...
	public static Node smallerEqualBigger(Node head, int value) {
		...
		// 链接各个区域
		if(LH != null) { // 如果有小于区
			LT.next = EH; // 小于区的尾连接等于区。不管有没有等于区，都不会报错，因为等于区没有的话，ET就是null
			ET = (ET == null) ? LT : ET; // 等于区如果没有，则将等于区结尾赋值成小于区结尾，保证ET一定是正确的
		}
		if(ET != null) { // 如果小于去和等于区不是都没有，则用正确的ET去连BH
			ET.next = BH; // 小于区的尾连接大于区的头
		}
