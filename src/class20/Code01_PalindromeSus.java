...
public class Code01_PalindromeSus {
	...
	/**
	 * 最长回文子序列 给定一个字符串str，返回这个字符串的最长回文子序列长度 比如 ： str = “a12b3c43def2ghi1kpm”
	...
	// 范围尝试模型
	/**
	 * 一个函数f，给你str，在L~R范围上，给我返回最长回文子序列的长度是多少？ base case： L==R，
	 * 只有一个字符，最长回文子序列长度就是1，返回1 L==R-1，剩下两个字符，如果相等，则返回2，否则返回1（这个base case可以不补）
	 */
	...
	public static int longestPalindromeSubseq2(String s) {
		...
		char[] str = s.toCharArray();
		int N = str.length;
		int dp[][] = new int[N][N]; // dp[i][j]: i到j范围上最长回文子序列的长度是多少？
		dp[N - 1][N - 1] = 1; // 1个字符
		// 填好两条对角线
		for (int i = 0; i < N - 1; i++) {
			dp[i][i] = 1;
			dp[i][i + 1] = str[i] == str[i + 1] ? 2 : 1; // 两个字符
		}
		// 从下往上，从左往右填
		for (int L = N - 3; L >= 0; L--) {
			for (int R = L + 2; R < N; R++) {
...
//              int p4 =dp[L + 1][R - 1]; // 左下这个格子不用比拼，因为求当前格子的值的时候，会和它的左边比拼
                // 但是它的左边的格子在求的时候，已经和它的下面格子(也就是当前格子的左下)比拼过了
