...
public class Code01_FindMinKth {
	/**
	 * 在无序数组中找到第K小的数 1. 改写快排 2. bfprt算法
	 *
	 */
	/**
	 * 1. 改写快排 在快排过程中，做完一次partition，
	 * 小于V的放左边，等于V 放中间，大于v的放右边，然后查看要求的K在不在等于区的范围内
	 * 如果在，则命中，第K小的数就是V，
	 * 如果不在，如果K比等于区的 左边界小，则继续去小于区找；否则去大于区找，递归下去。
	 */
	...
	/**
	 * bfprt算法 1. 将arr中的数划分成五个数一组 2. 将每个单独的数组，排好序 3.将每个小数组的中位数取出来组成m
	 * 4.求出m数组的中位数，作为划分值p ==> m数组长度是N / 5, 那么它的中位数就是第 N / 10小的数，所有可以调用bfprt(m, N /
	 * 10)求 5.用p作为划分值，对原数组进行划分 6.查看要求的K是否在等于区范围内，在的话，直接返回，不在的话，去小于区或者大于区找,
	 * 继续递归调用bfprt
	 */
